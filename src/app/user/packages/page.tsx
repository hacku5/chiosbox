      ...
      {steps.map((step, i) => {
        ...
        return (
          <div key={step.key} className="relative flex-1 flex flex-col items-center">
            ...
            <div className={`relative z-10 w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold ${isCurrent
...
export default function PackagesPage() {
  ...
  const packages = tourRunning ? (mockPackages as unknown as Package[]) : realPackages;
  ...
  if ((authLoading || loading) && !tourRunning) return <PackageListSkeleton />;
  ...
  return (
    <div className="p-6 lg:p-8">
      <div className="max-w-5xl mx-auto" data-tour="package-list">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
          ...
          <Link href="/user/actions" className="inline-flex items-center gap-2 px-5 py-2.5 bg-chios-purple text-white text-sm font-semibold rounded-full hover:bg-chios-purple-dark transition-colors duration-200 shadow-md cursor-pointer">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
        ...
        <div className="flex gap-2 mb-6 overflow-x-auto pb-2">
          {filterTabs.map((tab) => (
            <button
              ...
              className={`px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap transition-all duration-200 cursor-pointer ${filter === tab.key
        ...
        <motion.div layout className="space-y-3">
          {filtered.length === 0 && (
            <div className="text-center py-16 text-deep-sea-teal/30">
              ...
              {!search.trim() && (
                <Link href="/user/actions" className="inline-block mt-3 text-sm text-chios-purple font-medium hover:underline cursor-pointer">
                  {t("packages.notFound")}
                </Link>
              )}
            </div>
            ...
            {filtered.map((pkg) => {
              ...
              return (
                <motion.div
                  ...
                  data-tour={filtered.indexOf(pkg) === 0 ? "package-card" : undefined}
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-wrap gap-2">
                        ...
                        {pkg.status === "depoda" && (
                          <span className={`inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-md ${pkg.free_days_left <= 5
                              ? "text-accent-orange bg-sunset-gold/10"
                              : "text-deep-sea-teal/50 bg-deep-sea-teal/[0.03]"
                            }`}>
                            {t("packages.daysFree", { days: String(pkg.free_days_left) })}
                          </span>
                        )}
                        {Number(pkg.demurrage_fee) > 0 && (
