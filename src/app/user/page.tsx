...
export default function DashboardPage() {
  ...
  const mockInvoices = useTourStore((s) => s.mockInvoices);
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <motion.div
            ...
            data-tour="dashboard-storage"
          >
            <h2 className="font-display text-lg font-semibold text-deep-sea-teal mb-4">
              {t("dashboard.storagePeriod")}
            </h2>
            <StorageCountdown daysLeft={minFreeDays || 0} packageCount={depodaCount} />
            {depodaCount === 0 ? (
              <p className="mt-3 text-sm text-success-green font-medium">
                {t("dashboard.storageAllClear")}
              </p>
            ) : overduePackages.length > 0 ? (
              <>
                <p className="mt-3 text-sm text-deep-sea-teal/70">
                  {overduePackages.length} {t("common.package")} · €{overdueTotal.toFixed(2)}
                </p>
                <motion.div
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  className="mt-3 px-3 py-1.5 bg-danger-red/10 rounded-full"
                >
          ...
          <motion.div
            ...
            data-tour="dashboard-packagestats"
          >
